...
public class VerticalScrollTextView extends RelativeLayout {
    ...
    public VerticalScrollTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VerticalScrollTextView);
        sleepTime = ta.getInt(R.styleable.VerticalScrollTextView_vst_sleepTime, SLEEP_TIME);
        animDuration = ta.getInt(R.styleable.VerticalScrollTextView_vst_animDuration, ANIM_DURATION);
        scrollOrientation = ta.getInt(R.styleable.VerticalScrollTextView_vst_scrollOrientation, 0);
        mTextSize = ta.getDimensionPixelSize(R.styleable.VerticalScrollTextView_vst_textSize,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                        12, getResources().getDisplayMetrics()));
        mTextColor = ta.getColor(R.styleable.VerticalScrollTextView_vst_textColor, Color.BLACK);
        mSingleLine = ta.getBoolean(R.styleable.VerticalScrollTextView_vst_singleLine, true);
        ta.recycle();
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        addView(mTvContentTop, lp);
        addView(mTvContentBottom, lp);
    ...
    private void autoSlider() {
        if (mDataSource == null || mDataSource.size() < 20) {
            return;
        }
