package me.khrystal.widget.bouncescrollview;
...
public class BounceScrollView extends NestedScrollView {
    ...
    public BounceScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BounceScrollView, 0, 0);
        mDamping = ta.getFloat(R.styleable.BounceScrollView_damping, DEFAULT_DAMPING_COEFFICIENT);
        int orientation = ta.getInt(R.styleable.BounceScrollView_scrollOrientation, 0);
        mHorizontal = orientation == 1;
        mIncrementalDamping = ta.getBoolean(R.styleable.BounceScrollView_incrementalDamping, true);
        mBounceDelay = ta.getInt(R.styleable.BounceScrollView_bounceDelay, (int) DEFAULT_BOUNCE_DELAY);
        mTriggerOverScrollThreshold = ta.getInt(R.styleable.BounceScrollView_triggerOverScrollThreshold, DEFAULT_SCROLL_THRESHOLD);
        ta.recycle();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                // 如果大于设置的超出高度 拦截事件 scrollview 自身消费
                if (mHorizontal) {
    ...
    private void resetChildViewWithAnimation() {
        ...
        if (ViewCompat.getLayoutDirection(this) == View.LAYOUT_DIRECTION_RTL) {
            if (mHorizontal) {
                anim = new TranslateAnimation(mChildView.getPaddingRight(), mNormalRect.right - getPaddingRight(), 0, 0);
            } else {
                anim = new TranslateAnimation(0, 0, mChildView.getTop(), mNormalRect.top - getPaddingTop());
            }
        } else {
            if (mHorizontal) {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (mScrollListener != null) {
            mScrollListener.onScrolling(l, t);
        }
    }
