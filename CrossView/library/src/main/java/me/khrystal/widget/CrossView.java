...
public class CrossView extends View {
    ...
    private float[] mTopXY;

    /**
     * Internal state flag for the drawn appearance, plus or cross;
     * The default starting position is "plus". This represents the real configuration,
     * whereas {@code mPercent} holds the frame-by-frame position when animation between
     * the states
     */
    private int mState = FLAG_STATE_PLUS;

    /**
     * The percent value upon the arcs that line endpoints should be found when drawing
     */
    ...
    private void readXmlAttributes(Context context, AttributeSet attrs) {
        ...
        try {
            mColor = ta.getColor(R.styleable.CrossView_lineColor, DEFAULT_COLOR);
        } finally {
            ta.recycle();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        setPointFromPercent(mArcBottom, mArcLengthBottom, mPercent, mTopXY);

        canvas.drawLine(mFromXY[0], mFromXY[1], mTopXY[0], mTopXY[1], mPaint);

        setPointFromPercent(mArcLeft, mArcLengthLeft, mPercent, mFromXY);
        setPointFromPercent(mArcRight, mArcLengthRight, mPercent, mTopXY);
    ...
    /**
     * Perform measurements and pre-calculations.
     * This should be called any time the view measurements or
     * visuals are changed, such as with a call to
     * {@link #setPadding(int, int, int, int)}
     * or an operating system callback like
    ...
    static class CrossViewState extends BaseSavedState {
        ...
        public CrossViewState(Parcelable superState) {
            super(superState);
        }

        public CrossViewState(Parcel in) {
            super(in);
