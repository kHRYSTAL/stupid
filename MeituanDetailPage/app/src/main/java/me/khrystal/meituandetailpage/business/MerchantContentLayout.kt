...
import me.khrystal.meituandetailpage.load
...
class MerchantContentLayout(context: Context, attrs: AttributeSet?) : ConstraintLayout(context, attrs) {
    var laySettle: MerchantContentLayout? = null
    ...
    private fun animViews(): Array<View> = arrayOf(laySimple, vAvatar, vMerchantName, layTicket,
            vTicket1, vTicket2, vSwitchIcon)
    ...
    fun switch(expanded: Boolean, byScrollerSlide: Boolean = false) {
        ...
        statesChangeByAnimation(animViews(), R.id.vs1, R.id.vs2, start, end, null,
                if (!byScrollerSlide) internalAnimListener else null, 300)

        laySettle?.switch(isExpanded)
    }

    var animListener: AnimatorListenerAdapter? = null
