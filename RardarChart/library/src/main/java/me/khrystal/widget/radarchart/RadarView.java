package me.khrystal.widget.radarchart;
...
import me.khrystal.widget.radarchart.util.AnimUtil;
import me.khrystal.widget.radarchart.util.RotateUtil;
...
public class RadarView extends View {
    ...
    private float mLayerOutLineWidth;
    ...
    private Paint mShapePointPaint;
    private float mPointRadius;
    private float mLineBlurWidth;
    private float mShapeBlurWidth;
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        // 线的圆点配置
        mLayerOutLineWidth = typedArray.getDimension(R.styleable.RadarView_layer_out_line_width, dp2px(1));
        mPointRadius = typedArray.getDimension(R.styleable.RadarView_line_point_radius, 0);
        mLineBlurWidth = typedArray.getDimension(R.styleable.RadarView_line_point_blur_width, dp2px(2));
        mPointColor = typedArray.getColor(R.styleable.RadarView_line_point_color, 0xFF9E9E9E);
    ...
    private void init() {
        ...
        if (mLineBlurWidth > 0)
            mLinePointPaint.setMaskFilter(new BlurMaskFilter(mLineBlurWidth, BlurMaskFilter.Blur.SOLID));

        if (mShapeBlurWidth > 0)
            mShapePointPaint.setMaskFilter(new BlurMaskFilter(mShapeBlurWidth, BlurMaskFilter.Blur.SOLID));
    ...
    public float getOutLayerLineWidth() {
        return mLayerOutLineWidth;
    }
    ...
    public void setLayerOutLineWidth(float width) {
        this.mLayerOutLineWidth = width;
        invalidate();
    }
    ...
    public void animeValue(int duration, RadarData data) {
        if (!mAnimUtil.isPlaying(data)) {
            mAnimUtil.animeValue(AnimUtil.AnimeType.ZOOM, duration, data);
        }
    }
    ...
    private void drawWeb(Canvas canvas) {
        for (int i = mLayer; i >= 1; i--) {
            ...
            if (i == mLayer && mLayerOutLineWidth > 0) {
                mRadarLinePaint.setColor(mLayerLineColor);
                mRadarLinePaint.setStrokeWidth(mLayerOutLineWidth);
                canvas.drawPath(mRadarPath, mRadarLinePaint);
            } else if (mLayerLineWidth > 0) {
                mRadarLinePaint.setColor(mLayerLineColor);
    ...
    private void drawVertex(Canvas canvas, int index, double angleSin, double angleCos) {
        float x = (float) (mPointCenter.x + angleSin * (mRadius + mVertexTextOffset * 3 / 2));
...
//            drawVertexImpl(canvas, text, null, mVertexTextPaint, x - textWidth / 2, y + textHeight / 4);
            drawVertexImpl(canvas, text, null, mVertexTextPaint, y + textHeight / 4, x - textWidth / 2);
        }
    }
    ...
    private void drawVertexLine(Canvas canvas, double angleSin, double angleCos) {
        ...
        if (mPointRadius > 0) {
            ...
            canvas.drawCircle(x, y, mPointRadius, mLinePointPaint);
        }
    ...
    private void drawData(Canvas canvas) {
        for (int i = 0; i < mRadarData.size(); i++) {
            ...
            // 画圆点
            for (int j = 0; j < textPoint.length; j++) {
                matrix.reset();
                matrix.postTranslate(textPoint[j].x - bitmap.getWidth() / 2, textPoint[j].y - bitmap.getHeight() / 2);
                canvas.drawBitmap(bitmap, matrix, mShapePointPaint);
                mShapePointPaint.setColor(mShapePointColor);
                canvas.drawCircle(textPoint[j].x, textPoint[j].y, mPointRadius, mShapePointPaint);
            }
