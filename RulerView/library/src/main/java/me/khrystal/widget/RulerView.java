    ...
    private void drawScaleAndNum(Canvas canvas) {
        ...
        while (rulerRight < width) {
            if (num1 % scaleCount == 0) {
                if ((moveX >= 0 && rulerRight < moveX - scaleGap) || width / 2 - rulerRight <= getWhichScaleMovex(maxScale + 1) - moveX) {   //去除左右边界
                ...
                if ((moveX >= 0 && rulerRight < moveX) || width / 2 - rulerRight < getWhichScaleMovex(maxScale) - moveX) {   //去除左右边界
