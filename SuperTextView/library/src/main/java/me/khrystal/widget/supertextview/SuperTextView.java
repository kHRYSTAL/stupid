...
import me.khrystal.widget.supertextview.sys_adjusters.PressAdjuster;
import me.khrystal.widget.supertextview.utils.STVUtils;
...
public class SuperTextView extends TextView {
    ...
    /**
     * inflate XML 布局文件时会被调用
     * @param context View 运行的Context环境
     ...
     * 在调用这个函数前，建议开发者根据当前所使用的图片框架实现{@link Engine}，
     ...
     * 在调用这个函数前，建议开发者根据当前所使用的图片框架实现{@link me.khrystal.widget.supertextview.image_engine.Engine}，
     * 然后通过{@link ImageEngine#install(Engine)}为SuperTextView的ImageEngine安装一个全局引擎，开发者可以在
     * {@link Application#onCreate()}中进行配置（需要注意任何时候新安装的引擎总会替换掉原本的引擎）。
     * 在未设置引擎的情况下，SuperTextView仍然会通过内置的一个十分简易引擎去下载图片。
     *
    ...
    /**
     * 状态图模式。SuperTextView定义了10种显示模式。他们控制这状态图的相对位置
     * 默认为居中
     */
