package me.khrystal.weyuereader.view.service;
...
import me.khrystal.weyuereader.db.helper.BookDownloadHelper;
...
import me.khrystal.weyuereader.view.base.BaseService;
...
//        return super.onStartCommand(intent, flags, startId);
        // 接受创建的DownloadTask
        Disposable disposable = RxBus.getsInstance()
                .toObservable(DownloadTaskBean.class)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(taskBean -> {
                    if (TextUtils.isEmpty(taskBean.getBookId()) || !checkAndAlterDownloadTask(taskBean)) {
        ...
        Disposable deleteDisp = RxBus.getsInstance()
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(deleteTaskEvent -> {
                    // 判断是否该数据存在加载列表中
                    boolean isDelete = true;
                    for (DownloadTaskBean taskBean : mDownloadTaskQueue) {
                        if (taskBean.getBookId().equals(deleteTaskEvent.collBook.get_id())) {
                            isDelete = false;
                            break;
                        }
                    }
                    ...
                    // 返回状态
                    RxBus.getsInstance().post(new DeleteResponseEvent(isDelete, deleteTaskEvent.collBook));
                });
    ...
    private void postMessage(String msg) {
        RxBus.getsInstance().post(new DownloadMessage(msg));
    }
