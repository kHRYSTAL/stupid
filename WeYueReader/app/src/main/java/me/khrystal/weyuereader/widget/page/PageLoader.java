...
public abstract class PageLoader {
    ...
    void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            // 初始化标题参数
            // 需要注意 绘制text的y的起始点是text的基准线的位置 而不是text的头部的位置
            ...
            // 擦除区域
            mBgPaint.setColor(mPageBg);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + ScreenUtils.dpToPx(2),
                    mDisplayWidth, mDisplayHeight, mBgPaint);
        }
...
//                });
    ...
    void pageCancel() {
        ...
        if (mCurPage.position == 0 && mCurChapterPos > mLastChapter) {
            ...
            // 加载上一章取消了(maybe has bug)
            nextChapter();
        }
        // 假设加载到下一页了, 又取消了 那么需要重新load
